...
def memory_tool(action: str = None, target: str = "memory", content: str = None, old_text: str = None,
    ...
    if os.environ.get("HERMES_MEMORY_EVAL", "").strip().lower() in {"1", "true", "yes", "on"}:
        if operations or action in _STORE_ACTIONS:
            return json.dumps({
                "success": True, "done": True, "skipped": True,
...
MEMORY_SCHEMA = {
    ...
    "description": (
        ...
        "Replacing an entry promotes repeated corrections, making them more prominent. "
        "P3 entries render as iron rules. Use [Pn] only for corrections that keep being ignored.\n\n"
