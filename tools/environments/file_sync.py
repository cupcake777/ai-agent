...
def _safe_extract_remote_tar(tar: tarfile.TarFile, staging: str) -> None:
    """Extract ordinary files/directories without trusting remote paths or metadata."""
    root = Path(staging).resolve()
    members = tar.getmembers()
    expanded_bytes = 0
    for member in members:
        raw_name = member.name
        posix_name = PurePosixPath(raw_name.replace("\\", "/"))
        ...
        if member.isfile():
            ...
            mode |= 0o700
        member.mode = mode
    # Keep our preflight checks for stable errors/size accounting and also use
    # Python's maintained extraction policy as defense in depth.
