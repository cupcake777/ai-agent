...
SKILL_MANAGE_SCHEMA = {
    ...
    "description": (
        ...
        "=== SKILL vs MEMORY: strict boundary ===\n"
        "Skill = REUSABLE MULTI-STEP PROCEDURE that will be called repeatedly across sessions.\n"
        "Memory = FACTS, PREFERENCES, CORRECTIONS, ENVIRONMENT DETAILS to learn.\n"
        "If it won't be invoked as a workflow in future sessions, it's memory, not a skill.\n\n"
        "Create skill ONLY when ALL conditions met:\n"
        "  1. User explicitly requests \"remember this workflow/procedure\", OR\n"
        "     the task is a complex multi-step procedure that WILL repeat (deploy service, configure env)\n"
        "  2. The procedure spans 3+ distinct steps with specific commands/API calls\n"
        "  3. It will be reused across multiple future sessions (not a one-off)\n\n"
        "NEVER create skill for (use memory instead):\n"
        "  - One-off error fixes, debugging discoveries → memory\n"
        "  - User corrections or preference changes → memory\n"
        "  - Knowledge/facts learned in conversation → memory\n"
        "  - One-time debugging sessions → memory\n"
        "  - Short-term status or state → nothing\n\n"
        "Update when: instructions stale/wrong, OS-specific failures, "
        "missing steps or pitfalls found during use. "
        "If you used a skill and hit issues not covered by it, patch it immediately.\n\n"
        "Confirm with user before creating ANY skill. Skip for simple one-offs.\n\n"
