...
class _ApprovalEntry:
    ...
    def __init__(self, data: dict):
        ...
        self.data = dict(data)
        request_id = self.data.get("request_id") or self.data.get("approval_id") or uuid.uuid4().hex
        self.data["request_id"] = request_id
        self.data.setdefault("approval_id", request_id)
...
def _await_gateway_decision(session_key: str, notify_cb, approval_data: dict, *, surface: str = "gateway") -> dict:
    ...
    # Keep the legacy hook alias for the installed approval bridge while the
    # canonical gateway payload uses ``request_id``.
    payload["approval_id"] = entry.data["request_id"]
    with _approval._lock:
