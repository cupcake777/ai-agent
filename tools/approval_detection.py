...
def _home_prefix_fold_regex(path: str):
    ...
    bare home is never folded. A POSIX single-component home such as ``/root`` is valid;
    unset paths, ``/``, and drive roots remain degenerate so a stray HOME cannot rewrite
    unrelated prefixes."""
    components = [c for c in re.split(r"[/\\]+", path) if c] if path else []
    posix_single_component = len(components) == 1 and path.startswith("/")
    if not components or (len(components) < 2 and not posix_single_component):
        return None
    # A one-component POSIX home must keep its leading slash; otherwise ``root/foo``
    # in ordinary argument text could be mistaken for ``/root/foo``.
    leading = r"/+" if posix_single_component else r"[/\\]*"
    return re.compile(leading + r"[/\\]+".join(re.escape(c) for c in components) + _PATH_TAIL)
