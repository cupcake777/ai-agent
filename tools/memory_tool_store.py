...
class MemoryStore:
    ...
    def _set_priority(entry: str, priority: int) -> str:
        priority = max(DEFAULT_PRIORITY, min(int(priority), MAX_PRIORITY))
        stripped = PRIORITY_RE.sub("", entry).strip()
        return stripped if priority == DEFAULT_PRIORITY else f"[P{priority}] {stripped}"

    def _autoboost_priority(self, old_entry: str, new_content: str) -> str:
        return self._set_priority(
            new_content,
            max(
                self._parse_priority(new_content),
                min(self._parse_priority(old_entry) + 1, MAX_PRIORITY),
                1,
            ),
        )
    ...
    def _char_limit(self, target: str) -> int:
        fallback = self.user_char_limit if target == "user" else self.memory_char_limit
        try:
            from hermes_cli.config import read_user_config_raw
            section = (read_user_config_raw() or {}).get("memory", {}) or {}
    ...
    def _edit(self, target: str, old_text: str, new_content: Optional[str]) -> Dict[str, Any]:
        ...
        def _apply(entries, limit):
            ...
            replacement = None if new_content is None else self._autoboost_priority(entries[idx], new_content)
            replaced = entries[:idx] + ([] if replacement is None else [replacement]) + entries[idx + 1:]
    ...
    def _apply_batch_op(self, working: List[str], act: str, content: str, old_text: str, pos: str) -> Optional[str]:
        ...
        working[idx:idx + 1] = (
            [self._autoboost_priority(working[idx], content)]
            if act == "replace" else []
        )
        return None
    ...
    def _render_block(self, target: str, entries: List[str]) -> str:
        """System prompt block with priority ordering and an optional injection-only cap."""
        if not entries:
            return ""
        rendered = []
        for entry in sorted(entries, key=lambda item: -self._parse_priority(item)):
            priority = self._parse_priority(entry)
            text = self._strip_priority(entry)
            rendered.append(
                f"⚠⚠ {text}" if priority >= 3
                else f"⚠ {text}" if priority == 2
                else f"▷ {text}" if priority == 1
                else text
            )
        full_content = ENTRY_DELIMITER.join(rendered)
        content = full_content
        try:
            from hermes_cli.config import load_config_readonly
            section = (load_config_readonly() or {}).get("memory", {}) or {}
            key = "user_prompt_char_limit" if target == "user" else "memory_prompt_char_limit"
            prompt_limit = int(section.get(key, 0) or 0)
        except Exception:
            prompt_limit = 0
        if prompt_limit and len(content) > prompt_limit:
            kept, used, omitted = [], 0, 0
            for entry in rendered:
                extra = len(entry) + (len(ENTRY_DELIMITER) if kept else 0)
            ...
            if kept:
                ...
                if omitted:
                    content += (
                        f"{ENTRY_DELIMITER}[Prompt injection truncated: {omitted} lower-priority "
                        "entries omitted from this request only; use memory/session_search when needed.]"
                    )
        sep = "═" * 46
        title = MEMORY_BLOCK_HEADERS["user" if target == "user" else "memory"]
        return f"{sep}\n{title} [{self._usage_pct(target, len(full_content))}]\n{sep}\n{content}"
