...
def _home_prefix_fold_regex(path: str):
    ...
    # Require a real directory prefix, not a filesystem root. POSIX homes can
    # legitimately be a single component below root (``/root`` in containers or
    # on servers). A Windows single component is only a drive root, not a home.
    posix_single_component_home = path.startswith("/") and len(components) == 1
    if len(components) < 2 and not posix_single_component_home:
        return None
...
def resolve_gateway_approval(session_key: str, choice: str,
    ...
    *reason* is an optional free-text explanation attached to an explicit
    ...
    ``approval_id`` is a compatibility alias for older approval plugins. New
    integrations should use ``request_id``; when both are provided,
    ``request_id`` is authoritative.

    Returns the number of approvals resolved (0 means nothing was pending).
    """
    request_id = request_id or approval_id
    with _lock:
...
def _await_gateway_decision(session_key: str, notify_cb, approval_data: dict,
    ...
    _fire_approval_hook(
        ...
        approval_id=entry.data["request_id"],
        command=command,
        description=description,
        pattern_key=primary_key,
        pattern_keys=list(all_keys),
