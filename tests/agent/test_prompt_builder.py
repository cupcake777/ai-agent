...
class TestBuildSkillsSystemPrompt:
    ...
    def test_compact_all_config_demotes_every_category(self, monkeypatch, tmp_path):
        monkeypatch.setenv("HERMES_HOME", str(tmp_path))
        (tmp_path / "config.yaml").write_text("skills:\n  compact_all: true\n")
        for category, name, description in (
            ("social-media", "thread-writer", "Write threads"),
            ("research", "paper-reader", "Read papers"),
        ):
        ...
        compact = build_skills_system_prompt()
        assert "thread-writer" in compact and "paper-reader" in compact
        assert "Write threads" not in compact and "Read papers" not in compact
        category_lines = [line for line in compact.splitlines() if "[names only]:" in line]
        assert len(category_lines) == 2
