...
def test_live_session_payload_replays_pending_approval(server, monkeypatch):
    ...
    # Both identifiers are injected by _ApprovalEntry: request_id is the
    # upstream wire name and approval_id is retained for desktop-plugin
    # compatibility. They must identify the same queued request.
    request_id = replayed.pop("request_id")
    approval_id = replayed.pop("approval_id")
    assert request_id
    assert approval_id == request_id
