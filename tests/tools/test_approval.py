...
class TestSensitiveInPlaceEditPattern:
    ...
    def test_posix_single_component_root_home_is_folded(self, monkeypatch):
        monkeypatch.setenv("HOME", "/root")
        dangerous, key, _ = detect_dangerous_command(
            "sed -i 's/a/b/' /root/.bashrc"
        )
        assert dangerous is True
        assert key is not None

    def test_filesystem_root_is_not_folded_as_home(self, monkeypatch):
        monkeypatch.setenv("HOME", "/")
        dangerous, key, _ = detect_dangerous_command(
            "sed -i 's/a/b/' /tmp/ordinary.txt"
        )
        assert dangerous is False
        assert key is None
...
class TestApprovalTimeoutIsNotConsent:
    ...
    def setup_method(self):
        ...
        self._session_token = approval_context.set_current_session_key(self.SESSION_KEY)

    def teardown_method(self):
        from tools import approval as mod
        mod._gateway_queues.clear()
        mod._gateway_notify_cbs.clear()
        approval_context.reset_current_session_key(self._session_token)
    ...
    def test_timeout_blocks_with_no_consent_and_timeout_hook(self, monkeypatch):
        ...
        pres = [c for c in hook_calls if c[0] == "pre_approval_request"]
        assert pres, "pre_approval_request hook did not fire"
        request_id = notified[0]["request_id"]
        assert pres[-1][1]["request_id"] == request_id
        assert pres[-1][1]["approval_id"] == request_id
        assert posts[-1][1]["request_id"] == request_id
        assert posts[-1][1]["approval_id"] == request_id
    ...
    def test_stale_request_id_cannot_resolve_current_approval(self, monkeypatch):
        ...
        assert mod.resolve_gateway_approval(
            self.SESSION_KEY, "deny", approval_id=request_id
        ) == 1
        thread.join(timeout=5)
