...
class TestFireStreamDeltaEmptiness:
    ...
    def test_repetition_guard_delivers_notice_once_and_swallows_tail(self):
        from agent.stream_repetition_guard import (
            StreamRepetitionGuardConfig, StreamingRepetitionGuard,
        )
        agent, delivered = _agent_with_sink()
        agent._stream_repetition_guard = StreamingRepetitionGuard(
            StreamRepetitionGuardConfig(chunk_repeat_max=2, min_pattern_len=1000)
        )
        for _ in range(4):
            agent._fire_stream_delta("same")
        agent._fire_stream_delta("tail")
        assert sum("repetition detected" in item for item in delivered) == 1
