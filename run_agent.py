...
class AIAgent:
    ...
    def _build_keepalive_http_client(base_url: str = "", *, verify: Any = True) -> Any:
        ...
        try:
            ...
            # injects bot-identifying headers on every request, which some
            # Cloudflare rules interpret as bot traffic. httpx event hooks fire
            # after the request is built but before it is sent, so they can
            # override headers injected by the SDK.
