...
def _resolve_custom_chat_credentials(provider_name: str) -> Dict[str, str]:
    ...
    try:
        ...
        return {
            "provider": pool_key,
            "api_key": str(entry.runtime_api_key or "").strip() if entry else "",
            "base_url": str(entry.runtime_base_url or "").strip().rstrip("/") if entry else "",
        }
...
class XAIWebSearchProvider(BaseWebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        if _configured_mode(cfg) == "chat_completions":
            creds = _resolve_custom_chat_credentials(
                cfg.get("provider") or DEFAULT_CUSTOM_PROVIDER
            )
    ...
    def _chat_completions_search(
        ...
        if not api_key or not base_url:
            return _fail(
                f"No credentials found for {provider_name}. Check web.xai.provider in config.yaml."
            )
        timeout = _coerce(float, cfg.get("timeout", DEFAULT_TIMEOUT), DEFAULT_TIMEOUT)
        prompt = (
            "You are a web search engine. Find current information for the query below and "
            f"return at most {limit} sources, each with its full https:// URL and a 1-2 sentence "
            f"summary. If possible return the JSON schema {{\"results\": [{{\"title\": \"string\", "
        ...
        try:
            ...
            return _fail(
                f"Custom web search returned HTTP {status}: {detail}" if status
                else f"Could not reach endpoint: {exc}"
            )
        content = self._chat_completion_content(resp)
        return search_ok(self._chat_results_from_text(content, limit=limit) if content else [])
    ...
    @classmethod
    def _chat_results_from_text(cls, text: str, *, limit: int) -> List[Dict[str, Any]]:
        if parsed := cls._try_parse_json_results(text, limit=limit):
            return parsed
        seen: set[str] = set()
        ...
        for index, line in enumerate(lines):
            for raw_url in _URL_RE.findall(line):
                ...
                context = " ".join(
                    part for part in (
                        line.strip(), lines[index + 1].strip() if index + 1 < len(lines) else ""
                    ) if part
                )[:300]
                rows.append(_row("", url, context, len(rows) + 1))
                if len(rows) >= limit:
                    return rows
        return rows
