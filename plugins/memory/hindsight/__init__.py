...
def _memory_eval_mode() -> bool:
    """True for benchmark subprocesses that may recall but must not retain."""
    return os.environ.get("HERMES_MEMORY_EVAL", "").strip().lower() in {
        "1", "true", "yes", "on",
    }
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def _apply_retain_policy(self, cfg: dict) -> None:
        ...
        if _memory_eval_mode() and self._auto_retain:
            ...
            logger.info(
                ...
                "(bank=%s remains readable)", self._bank_id,
            )
    ...
    def _tool_retain(self, args: dict) -> str:
        if _memory_eval_mode():
            return "Memory retain skipped: HERMES_MEMORY_EVAL is set, so benchmark runs are read-only."
