...
def check_email_requirements() -> bool:
    ...
    return all((address, password, imap)) and bool(smtp or resend)
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._resend_api_key = setting("RESEND_API_KEY", "resend_api_key").strip()
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing := [
            name for name, value in required
    ...
    def _send_via_resend(
        ...
        *, files: Optional[List[Tuple[Path, str]]] = None, lenient: bool = False,
        ...
        if original_msg_id:
            headers.update({"In-Reply-To": original_msg_id, "References": original_msg_id})
        payload: Dict[str, Any] = {
            "from": self._address, "to": [to_addr], "subject": subject, "text": body,
        }
        if headers:
        ...
        try:
            ...
            raise RuntimeError(
                f"Resend API failed with HTTP {exc.code}: {detail}"
            ) from exc
        try:
    ...
    def _send_with_files(self, to_addr: str, body: str, files: List[Tuple[Path, str]], *, lenient: bool,
        ...
        """Send a threaded reply with attachments, optionally skipping unreadable files."""
        if self._resend_api_key:
            return self._send_via_resend(
                to_addr, body, reply_to_msg_id, files=files, lenient=lenient)
