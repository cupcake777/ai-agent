...
def group_providers(slugs):
    ...
    for slug in slugs:
        ...
        if not normalized or normalized in norm_to_original:
            continue
        norm_to_original[normalized] = original
        ordered_norms.append(normalized)
    present = set(ordered_norms)
    group_members = {
    ...
    for s in ordered_norms:
        ...
        members = group_members.get(gid) or [norm_to_original[s]]
        if len(members) <= 1:
